...
function serve_static(request, response){
	...
	fs.exists(filename, function(exists){
		...
		console.log(exists?"      OK":"NOTFOUND","---",filename);
...
//TODO: figure how and WHETHER that one works!
...
function run(){
	...
	var proxyUrl = 'http://localhost:' + SERVER_PORT + '/static/';
	var proxyParameter = '?' + CLIENT_PARAM_NAME + '=' + proxyUrl;
	var sampleProjectUrl = 'http://dev.excentos.com/SportScheck/DE/app_runner/activesports.start';
	var url = sampleProjectUrl + proxyParameter;

	console.log('\n\nuse the following URL parameter to invoke the proxy:\n', proxyParameter);
	console.log('\n\nopening >A SAMPLE< browser URL...\n',url);

	setTimeout(function(){open(url)}, 3000);
}
